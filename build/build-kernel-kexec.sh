#!/bin/bash
#
# Obraz docelowy kexec - konfiguracja obcieta do minimum (issue #92 autora): ten sam kernel, ale
...
# Obciecie konfiguracji. Cel ma zrobic dokladnie tyle: wstac, pokazac sie na
# ekranie, podniesc siec i wpuscic SSH. Kazdy megabajt zdjety z rozpakowanego
# jadra to megabajt mniej, ktory kexec_load musi zaalokowac w jadrze - a to
# jest ta polowa problemu, ktorej swap nie rusza.
for opt in IPV6 NETFILTER SOUND DRM USB_SUPPORT BLK_DEV_INITRD_COMPRESS_LZ4 	   WLAN BT MEDIA_SUPPORT INPUT_JOYDEV KALLSYMS_ALL DEBUG_INFO 	   NLS_DEFAULT_UTF8 HID_GENERIC USB_HID SCSI SCSI_LOWLEVEL 	   MSDOS_FS FAT_FS EXT4_FS BTRFS_FS XFS_FS NFS_FS; do
